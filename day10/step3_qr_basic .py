...
while True:
    ...
    for qr in qr_codes:
        # TODO: qr.type이 'QRCODE'인지 확인 (바코드는 제외)
        if qr.type != 'QRCODE':
            continue
        ...
        # TODO: draw_qr()으로 그리기
        draw_qr(frame, qr, COLOR_QR)

        # ==========================================
        # 9️⃣ 중복 감지 방지 (새로운 QR만 처리)
        # ==========================================

        # TODO: data가 detected_set에 없으면:
        #       1. detected_set에 추가
        #       2. 콘솔에 [감지 QR] {data} 출력
        #       3. SAVE_LOG=True면 파일에 기록
        #       4. AUTO_OPEN_URL=True이고 URL이면 브라우저 열기
        if data not in detected_set:
            detected_set.add(data)
            print(f"[감지 QR] {data}")

            if SAVE_LOG:
                log_file.write(f"QRCODE\t{data}\n")
                log_file.flush()
    ...
    cv2.putText(frame, f'Total QR: {len(detected_set)}', (10,30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
    ...
    if key == ord('q'):
        ...
        detected_set.clear()
        print("[Reset] Detection history cleared")
...
print(f"\nTotal {len(detected_set)} QR codes detected:")
