...
# ========== Step 1: 이미지 로드 ==========
img1 = cv.imread(get_sample('capture.png'), cv.IMREAD_GRAYSCALE)
img2 = cv.imread(get_sample('pic.jpg'), cv.IMREAD_GRAYSCALE)
...
if len(good_matches) >= MIN_MATCH_COUNT:
    ...
    if M is not None:
        ...
        # 투시 변환으로 img2 내 위치 계산
        dst = cv.perspectiveTransform(pts, M)

        # ========== Step 2: 결과 시각화 ==========
        # img2 컬러로 변환 후 사각형 그리기
        ...
        # inlier 매칭선 시각화
        match_img = cv.drawMatches(img1, kp1, img2, kp2, good_matches, None, **draw_params)
